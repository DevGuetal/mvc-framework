...
var Framework = function() {
	...
	return {
		create: function (args) {
			...
			//2 way data binding
			var dataBindingItems = html.match(/data-bind="(.*?)"/g);

			var eventObserver = Object.create (Observer.prototype);
			_extends(eventObserver, new Observer());
			
			for(var i = 0; i < dataBindingItems.length; i++) {
				var currentItem = dataBindingItems[i];
				var events = currentItem.substring('data-bind="'.length, currentItem.length - 1);
				...
				var callback = function(evt) {
					if(evt) {
						...
						if (events && events !== "") {
							ctrl.getModel()[events] = target.value;
							dataObserver.notify("{{"+events+"}}");
						}
					}
				}

				eventObserver.onDataChanged('keyup', callback);
			}
			
			// event binding
//			var eventItems = html.match(/data-event="(.*?)"/g);
//			for(var i = 0; i < eventItems.length; i++) {
//				var currentItem = eventItems[i];
